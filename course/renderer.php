...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function enrolment_options(stdClass $course, array $widgets, ?\core\url $returnurl = null): string {
        ...
        $templatecontext = [
            ...
            'coursebannerimage' => $course->bannerimageurl ?? 'https://edly-moodle.hibootstrap.com/theme/edly/pix/page-banner/ellipse.png',
        ];
    ...
    protected function course_name(coursecat_helper $chelper, core_course_list_element $course): string {
        ...
        if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
            if ($course->has_summary() || $course->has_course_contacts() || $course->has_course_overviewfiles()
                ...
                $content .= html_writer::link( $image, $url, ['title' => $this->strings->summary]);
    ...
    protected function buy_now_button(coursecat_helper $chelper, core_course_list_element $course): string {
    $content = '';
    // Define the button tag based on display mode.
    if ($chelper->get_show_courses() >= self::COURSECAT_SHOW_COURSES_EXPANDED) {
        $buttontag = 'a';
    ...
    $producturl = new moodle_url('/course/view.php', ['id' => $course->id]);
     $buttontext = 'Buy Now';
    // Create the button or link with the product URL.
    $buttonlink = html_writer::tag($buttontag, $buttontext, [
        'href' => $producturl->out(),
        'class' => $buttonclass . ($course->visible ? '' : ' dimmed'),
        'title' => $buttontext
    ]);
    $content .= html_writer::tag('div', $buttonlink, ['class' => 'buynow']);
    return $content;
}
    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        ...
        //          'src' => 'https://img.freepik.com/free-vector/hand-drawn-online-college-template-design_23-2150574159.jpg',
        //          'class' => 'banner-image',
        //          'alt' => 'Banner Image'
        // ));
        ...
        // .coursebox
        //  $content .= html_writer::empty_tag('div', array(
        //          'src' => 'https://img.freepik.com/free-vector/hand-drawn-online-college-template-design_23-2150574159.jpg',
        //          'class' => 'banner-image',
        //          'alt' => 'Banner Image'
        // ));
        ...
        $content .= $this->get_course_image( $course);
        // $content .= $this->coursecat_coursebox_content($chelper, $course);
      $content .= $this->course_name($chelper, $course);
    
        $content .= $this->get_course_summary_text($chelper, $course);
        // $content .= $this->course_summary($chelper, $course);
       
        $content .= $this->course_contacts($course);
         
        $content .= $this->buy_now_button($chelper, $course);
...
// protected function get_course_summary_content(coursecat_helper $chelper, core_course_list_element $course): string {
//     if ($course->has_summary()) {
//         return $chelper->get_course_formatted_summary($course, [
//             'overflowdiv' => true,
//             'noclean' => true,
//             'para' => false
//         ]);
...
// protected function course_summary(coursecat_helper $chelper, core_course_list_element $course): string {
//     $summarycontent = $this->get_course_summary_content($chelper, $course);
//     if (!empty($summarycontent)) {
//         return html_writer::div($summarycontent, 'summary');
//     }
//     return '';
// }


protected function get_course_image(core_course_list_element $course): string {
    global $CFG;
    ...
    if ($course->has_course_overviewfiles()) {
        foreach ($course->get_course_overviewfiles() as $file) {
            if ($file->is_valid_image()) {
                ...
                return html_writer::div($image, 'course-image-wrapper');
            }
        }
    }
...
protected function get_course_summary_text(coursecat_helper $chelper, core_course_list_element $course): string {
    if ($course->has_summary()) {
        // Temporarily clear overview files to prevent auto-injection of images
        $originalfiles = $course->get_course_overviewfiles();
        $course->summary = preg_replace('/<img[^>]*>/', '', $course->summary); // Just in case any <img> in summary
        
        // Clear overview files (temporary hack, since summary renderer may include them)
        $course->overviewfiles = [];

        // Now get summary without injected images
        ...
        return html_writer::div($summary, 'course-summary');
    }
    return '';
}
...
// }
    ...
    // protected function course_img(oursecat_helper $chelper, core_course_list_element $course): string {
    //      $content = '';
    //     if ($course->has_summary()) {
    //         $content .= $chelper->get_course_formatted_summary($course,
    //             array('overflowdiv' => true, 'noclean' => true, 'para' => false));
    ...
    protected function frontpage_remote_course(stdClass $course) {
        ...
        $output .= html_writer::empty_tag('img', array(
                 'src' => 'https://edly-moodle.hibootstrap.com/theme/edly/pix/page-banner/ellipse.png',
                 'class' => 'banner-image',
                 'alt' => 'Banner Image'
        ));
            ...
            switch ($v) {
                ...
                case FRONTPAGEALLCOURSELIST:
                    ...
                    $output .= $this->frontpage_part('skipavailablecourses', 'frontpage-available-course-list',
                        get_string('mycourses'), $availablecourseshtml);
                    break;
